...
document.querySelector('#weread-account').addEventListener('submit',async event=>{event.preventDefault();accountResult.textContent='';const form=new FormData(event.target);const account=form.get('account_id');const displayName=form.get('display_name');const path=account?`/api/admin/weread/accounts/${encodeURIComponent(account)}`:'/api/admin/weread/accounts';const response=await request(path,{method:account?'PUT':'POST',body:JSON.stringify({account_id:account?account:null,display_name:displayName||null,cookie_header:form.get('cookie_header'),access_expires_at:new Date(form.get('access_expires_at')).toISOString()})});if(response.ok){const value=await response.json();event.target.reset();accountResult.textContent=`Saved account ${value.account_id}; use this ID when adding a source.`}else{accountResult.textContent=await apiErrorMessage(response,'WeRead account could not be saved; check the values and try again.')}});
